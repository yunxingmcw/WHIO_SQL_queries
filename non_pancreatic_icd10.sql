USE [WHIO_Metadata]
GO
/****** Object:  StoredProcedure [MPri].[PROC_CREATE_TABLE_non_Pancreatic_Cancer_ICDDiagnosis]    Script Date: 2/10/2026 4:04:32 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROCEDURE [MPri].[PROC_CREATE_TABLE_non_Pancreatic_Cancer_ICDDiagnosis] 
AS
BEGIN
	...
	IF EXISTS (
		 ...
		 FROM SYS.OBJECTS 
		 WHERE 1=1
			 AND OBJECT_ID = OBJECT_ID(N'[MPri].[xFact_non_Pancreatic_Cancer_ICDDiagnosis]')
			 AND TYPE IN (N'U'))
	DROP TABLE [MPri].[xFact_non_Pancreatic_Cancer_ICDDiagnosis];
...
INTO MPri.xFact_non_Pancreatic_Cancer_ICDDiagnosis
FROM UniqueIDs u
